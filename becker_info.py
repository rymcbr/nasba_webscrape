'''
The purpose of this program is to scrape information from the Becker website 
to create a central repository of exam and licensure requirements, with an emphasis on
credit requirements.
...
# OPEN THE FILE
URL = "https://www.becker.com/cpa-review/requirements/"
becker_States = requests.get(URL, headers=HEADERS)


# MAKE A VARIABLE TO CONTAIN ALL OF THE CONTENTS
contents = becker_States.text
...
savedpage = open("beckermain.html",'w')
savedpage.write(str(soup))
savedpage.close()

# FIND THE TABLE WITH THE STATE LINKS
state_table = soup.find("div", id="StateList")
state_table_rows = state_table.find_all("a")

pprint.pprint(state_table_rows)
...
all_page_contents_list_df.to_csv('all_becker_info.csv', index=False)
pprint.pprint(all_page_contents_list)
'''
# GRAB ALL OF THE WEB FILES FROM LIST EXPORTED ABOVE

saved_export = pd.read_csv('nasba_webscrape/all_becker_info.csv')
saved_export.rename({'0': 'HTML'}, axis=1, inplace=True)
#pprint.pprint(saved_export)

#alabama, alaska, arizona, arkansas, california, colorado, connecticut, delaware, florida, georgia, hawaii, idaho, illinois, indiana, iowa, kansas, kentucky, louisiana, maine, maryland, massachusetts, michigan, minnesota, mississippi, missouri, montana, nebraska, nevada, newhampshire, newjersey, newmexico, newyork, northcarolina, northdakota, ohio, oklahoma, oregon, pennsylvania, rhodeisland, southcarolina, southdakota, tennessee, texas, utah, vermont, virginia, washington, westvirginia, wisconsin, wyoming = saved_export['HTML'].to_list()
...
for state in soup_dictionary.keys():
    text = soup_dictionary[state].find("main")
    text = text.find_all(["li","p"])
    text =  re.sub(r'<.+?>', '', str(text))
    text =  text.replace('\n','')
    text = re.sub(' +', ' ', text)
    exam_requirements[state] = text
    
#pprint.pprint(exam_requirements)



exam_requirements = pd.DataFrame.from_dict([exam_requirements])
exam_requirements = exam_requirements.transpose()
exam_requirements.set_index(state_links_df['HREF'],inplace=True)
pprint.pprint(exam_requirements)

exam_requirements.to_excel('nasba_webscrape/becker_requirements.xlsx')
state_links_df.to_excel('nasba_webscrape/becker_state_links.xlsx')
